...
# Storage class wrapper for your existing functions
class MilvusStorage:
    """
    Milvus storage class that wraps the existing function-based implementation.
    """
    
    def __init__(self, options=None):
        """Initialize MilvusStorage with configuration options."""
        self.options = default_options.copy()
        if options:
            self.options.update(options)
    
    def save(self, vcon_uuid: str, vcon_data: dict = None) -> bool:
        """
        Save a vCon to Milvus.
        
        Args:
            vcon_uuid: UUID of the vCon to save
            vcon_data: vCon data (optional, will retrieve from Redis if not provided)
            
        Returns:
            bool: True if successful, False otherwise
        """
        try:
            save_vcon_to_milvus(vcon_uuid, self.options)
            return True
        except Exception as e:
            logger.error(f"Failed to save vCon {vcon_uuid}: {e}")
            return False
    
    def get(self, vcon_uuid: str) -> Optional[dict]:
        """
        Get a vCon from Milvus.
        
        Args:
            vcon_uuid: UUID of the vCon to retrieve
            
        Returns:
            Optional[dict]: vCon data if found, None otherwise
        """
        return get_vcon_from_milvus(vcon_uuid, self.options)

# Rename the existing functions to avoid conflicts
def save_vcon_to_milvus(vcon_uuid: str, opts=default_options) -> None:
    """
...
def get_vcon_from_milvus(vcon_uuid: str, opts=default_options) -> Optional[dict]:
    """
...
# Keep backward compatibility with function-based interface
def save(vcon_uuid: str, opts=default_options) -> None:
    """Legacy function interface for saving vCons."""
    return save_vcon_to_milvus(vcon_uuid, opts)

def get(vcon_uuid: str, opts=default_options) -> Optional[dict]:
    """Legacy function interface for getting vCons."""
    return get_vcon_from_milvus(vcon_uuid, opts)

# Export the class for integration tests
__all__ = ['MilvusStorage', 'save', 'get']